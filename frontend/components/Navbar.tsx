...
import { ChevronDown, LayoutDashboard, LogOut, Menu, UserRound, UserCircle2, X } from 'lucide-react'
      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-2 px-3 sm:h-17 sm:gap-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-2 sm:gap-3 md:flex">
          ...
          {user ? (
            <div ref={profileMenuRef} className="relative">
              ...
              {isProfileMenuOpen ? (
                <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-2xl border border-outline-variant bg-surface p-2 shadow-2xl">
                  <Link href={dashboardHref} onClick={() => setIsProfileMenuOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-on-surface-variant transition hover:bg-surface-container hover:text-secondary">
                    <LayoutDashboard className="h-4 w-4" />
                    Dashboard
                  </Link>
                  <Link href="/profile" onClick={() => setIsProfileMenuOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-on-surface-variant transition hover:bg-surface-container hover:text-secondary">
            ...
            <Link href="/login" className="rounded-md bg-primary px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-primary/90 dark:bg-secondary dark:text-primary dark:hover:bg-secondary/90 sm:px-4 sm:py-2 sm:text-sm">
              {t('common.login')}
            </Link>
          )}
        </div>
            ...
            {user ? (
              <div className="mt-3 rounded-2xl border border-outline-variant bg-surface p-4">
                ...
                <div className="mt-4 grid gap-2">
                  <Link href={dashboardHref} onClick={() => setIsMobileMenuOpen(false)} className="inline-flex items-center justify-center rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-primary/90 dark:bg-secondary dark:text-primary dark:hover:bg-secondary/90">
                    Dashboard
                  </Link>
                  <Link href="/profile" onClick={() => setIsMobileMenuOpen(false)} className="inline-flex items-center justify-center rounded-xl border border-outline-variant px-4 py-3 text-sm font-semibold text-on-surface-variant transition hover:border-secondary hover:text-secondary">
                    Profile
                  </Link>
                  <button
                    type="button"
